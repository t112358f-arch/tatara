...
//! reference CPU 実装。
//!
//! GPU 側 (`#[kernel] fn sparse_ft_backward`) は **`experiments/002-fused-kernels/
//! src/main.rs` に inline 定義** されている (Stage 1-5 で確立した「`#[kernel]`
//! は bin entry に inline」制約)。本 module の `sparse_ft_backward_cpu` は GPU と
//! 同じロジックを host で素直に書き写したもので、Stage 2-7 (#43) GPU↔CPU 数値
//! 同等性テストの reference 用。
//!
//! ## アルゴリズム (bullet 上流 `linear/sparse.rs::SparseMatmulBwd::evaluate` に等価)
//!
//! sparse forward (Stage 2-6 #42) の対 backward。column-major weight layout
...
//! ```
//!
//! - **`grad_weight` の初期化**: 本 fn は **accumulate** semantics で、既存値に
//!   add する。host が呼び出し前に `device.memset(0)` (or `vec![0.0; ...]`) で
//!   zero clear する責務 (Stage 1-6 grad kernel と同 convention)
//! - bullet 上流 `evaluate` (`linear/sparse.rs:130-150`) は test 用に冒頭で
//!   `o.write(idx, zero)` を行うが、本 fn は production kernel semantics (accumulate)
//!   に揃えるため zero clear を含めない。test 側で初期化することで bullet 上流の
//!   expected 値を再現する
//! - layout: column-major weight (Stage 2-6 forward と同 `weight[idx * rows + ri]`)
//!
//! ## bullet 上流との対応 / divergence
//!
//! - bullet `evaluate_bwd` test (`linear/sparse.rs:256-269`、batch=2/rows=3/cols=3/
//!   nnz=4、grad_out=[0,1,2,3,4,5]、indices=[0,1,-1,-1,2,2,1,0]、expected=
//!   [3,5,7,3,5,7,6,8,10]) と同 fixture を本 PR の CPU test で 1:1 再現
//! - thread 配置: bullet 上流は PointwiseIR で per-row reduction、batch 軸別 unroll
//!   想定。本実装は **flat 1D `tid = bi * rows + ri`** で batch 軸も込み (Stage 2-6
//!   forward と同型 idiom、atomic scatter で衝突を吸収)
//! - `SparseMatmulBwdMulti` (`:158-235` の複数 backward 集約) は本 PR では非対応、
//!   Stage 3 trainer で必要になれば別 issue
//!
//! ## cuda-oxide 制限
//!
//! - GPU kernel は `+` / `*` / i32 比較 + `DeviceAtomicF32::fetch_add` で
//!   cuda-oxide 制限非該当 (Stage 1-6 grad と同 atomic scatter pattern)
//! - `f32::clamp` / `sqrt` 等は使わない
...
mod tests {
    ...
    /// bullet 上流 (`linear/sparse.rs::tests::evaluate_bwd`、`:256-269`) と同じ
    /// shape (batch=2, rows=3, cols=3, nnz=4) で完全一致。期待値
    ...
    /// する責務、Stage 1-6 grad と同 convention)。先行値 1.0 を入れて呼び出し、
    /// 結果が 「先行値 + 新加算分」 になることを確認。
    #[test]
    ...
    /// ことを確認 (Stage 3 trainer で autograd を組んだときの sanity check 用途)。
    #[test]
    fn forward_backward_consistency() {
        ...
        // 期待値: out = [2, 4, 10, 14] (Stage 2-6 forward test 同 fixture)
