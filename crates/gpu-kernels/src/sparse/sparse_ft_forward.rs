...
//!
//! GPU 側 (`#[kernel] fn sparse_ft_forward`) は **`experiments/002-fused-kernels/
//! src/main.rs` に inline 定義** されている (Stage 1-5 で確立した「`#[kernel]`
//! は bin entry に inline」制約)。本 module の `sparse_ft_forward_cpu` は GPU と
//! 同じロジックを host で素直に書き写したもので、Stage 2-6 (#42) GPU↔CPU 数値
//! 同等性テストの reference 用。
//!
